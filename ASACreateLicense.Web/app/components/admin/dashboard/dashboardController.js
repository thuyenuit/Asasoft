(function (app) {
    ...
    dashboardController.$inject = ['$scope', 'apiService', '$ngBootbox', 'notificationService', '$filter', '$state', 'Data']; // khởi tạo

    function dashboardController($scope, apiService, $ngBootbox, notificationService, $filter, $state, Data) {
        ...
        function viewAll() {
            Data.valueView = '1';
            $state.go('requestlicense');
        }

        $scope.viewAll2 = viewAll2;
        function viewAll2() {      
            Data.valueView = '1';
            $state.go('licenseinfolist');
        }
